...
def select_columns() -> None:
    ...
    def table_adapter(table: Table) -> None:
        ...
        if table.name == "family":
            ...
            table._columns.remove(table.columns["updated"])  # type: ignore[attr-defined]
...
def my_sql_via_pyarrow() -> None:
    ...
    def _double_as_decimal_adapter(table: sa.Table) -> None:
        ...
        for column in table.columns.values():
            if isinstance(column.type, sa.Double):  # type: ignore[attr-defined]
...
def use_type_adapter() -> None:
    ...
    def type_adapter(sql_type: TypeEngine[Any]) -> TypeEngine[Any]:  # type: ignore[type-arg]
